...
labels.gather_separation_torso='Separation at the gather (not scored)';
...
function render(id,result){
  ...
  if(game&&vision.ball_backend&&vision.ball_backend!=='mlx'&&vision.ball_backend_fallback)slower.push(`the fast ball detector isn't available, so detection ran on ${vision.ball_backend==='coreml'?'Core ML':'the CPU'}`);
  if(game&&vision.pose_backend_fallback)slower.push('Neural Engine pose isn\'t available, so pose ran in PyTorch');
  $('#speedNote').classList.toggle('hidden',!slower.length);
  $('#speedNote').textContent=slower.length?`Slower path: ${slower.join('; ')}. See “optional speedups” in the README.`:'';
